	...
	public ArrayList<Thing> defineHas() {
		...
		for(String thing:thingsList){//Percorre as tags e joga em thing
			...
			if(thingAndAtributes.length==1){//se o array tiver 1 termo, cria a coisa.
				...
				this.redundanceCheck++;
				double p = ProceduralGen.getDecimalNum(this.seed+this.redundanceCheck);//Um numero pseudorandomico
				
				if((p*100)<=ch){//Checa se ocorreu
					this.redundanceCheck++;
				...
				if((p*100)<=ch){//Checa se ocorreu
					...
					p = ProceduralGen.getDecimalNum(p);//Um numero pseudorandomico
					...
					int quant = min + (int)(p * ((max - min) + 1));//Calcula a quantidade de coisas a serem criadas.
